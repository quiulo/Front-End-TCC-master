...
import CreateModalPost from '../../components/Modal/CreateModalPost'; // Importe o componente CreateModalPost
...
function Home() {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const response = await api.get("/api/posts"); //ver no backend
                if (Array.isArray(response.data)) {
                    setPosts(response.data);
                } else {
                    console.error('Os dados dos posts não são um array:', response.data);
                }
            } catch (error) {
    ...
    return (
        <div className="flex bg-stone-100 py-12">
            <section className="flex flex-col gap-10 lg:pr-6 gap-5 flex-1">
                ...
                <PostList posts={posts} /> {/* Passando os posts como propriedade para o componente PostList */}
